...
async function fetchPrograms(){
  ...
  const url = `https://boatraceopenapi.github.io/programs/v2/${dateStr}.json`;
  ...
  data.programs.forEach(place=>{
    text += `${place.stadiumName} ： ${place.races.length}レース\n`;
  });
...
fetch("https://boatraceopenapi.github.io/programs/v2/20240206.json")
  .then(res => res.json())
  .then(data => {
    alert("② データ取得成功！");
    alert("開催場数：" + data.programs.length);
  })
  .catch(err=>{
    alert("❌ 通信失敗");
  });
