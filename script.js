...
function updateExpectationBars(base,predict,ai){
  ...
  document.querySelectorAll(".expectation-row").forEach((row,i)=>{
    ...
    [base[i],predict[i],ai[i]].forEach((val,j)=>{
      ...
      outer.appendChild(percentLabel);  // ←バーの右に固定配置
...
function updateBets(ai){
  ...
  // 残りの番号を取得
  const remaining=[1,2,3,4,5,6].filter(n=>![main,sub,third].includes(n));
  ...
  if(cols.length>=3){
    ...
    // 逃げ列（重複なし）
    setCol(cols[2],[
      `${main}-${remaining[0]}-${remaining[1]}`,
      `${main}-${remaining[1]}-${remaining[0]}`,
      `${main}-${remaining[2]}-${remaining[3]}`
    ]);
  }
}
