...
function updateExpectationBars(base,predict,ai){
  ...
  const colors=["#fff","#000","#ff0000","#0000ff","#ffff00","#00aa00"];
...
function updateBets(ai){
  ...
  const escapeTargets=[2,3,4,5,6];
  let escapeBets=[];

  for(let i=0;i<escapeTargets.length;i++){
    for(let j=0;j<escapeTargets.length;j++){
      if(i!==j){
        escapeBets.push(`1-${escapeTargets[i]}-${escapeTargets[j]}`);
      }
    }
  }

  escapeBets=escapeBets.slice(0,3);

  const all=[...coreBets,...escapeBets];
