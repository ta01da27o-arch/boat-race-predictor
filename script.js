...
// 追い風 → イン有利
if(windType === 1){
  if(styleType === "inner") tacticalFactor += 0.08;
  if(styleType === "outer") tacticalFactor -= 0.05;
}

// 向かい風 → 外伸び有利
if(windType === 2){
  if(styleType === "outer") tacticalFactor += 0.08;
  if(styleType === "inner") tacticalFactor -= 0.05;
}
...
if(windType === 0){
  tacticalFactor += 0;
    ...
    let a = Math.round(
      (b*0.45 + p*0.35 + trend[i]*0.2) * weatherFactor * courseFactor * startSkill * tacticalFactor
    );
