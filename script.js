...
function updateAnalysis(ai){
  ...
  let el = document.getElementById("analysisText");
  ...
  if(!el){
    ...
    document.getElementById("playerScreen").appendChild(el);
  }

  if(!window.expectedValues || window.expectedValues.length !== 6){
  ...
  if(maxEV >= 1.3){
    comment = `🔥 ${ev.indexOf(maxEV)+1}コース中心の期待値優勢`;
  }
  ...
  if(hitRateEl){
    hitRateEl.textContent = Number(window.hitExpectation).toFixed(1) + "%";
  }
}
// ===============================
// 買い目（重複完全排除）
