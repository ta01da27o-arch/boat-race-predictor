...
function calcAllWithTrend(stadiumIndex){
  ...
  for(let i=0;i<6;i++){
    ...
    b = Math.max(1,b); b = Math.min(100,b);
    p = Math.max(1,p); p = Math.min(100,p);

    let a = Math.round(b*0.45 + p*0.35 + trend[i]*0.2);
    a = Math.max(1,a); a = Math.min(100,a);
...
function updateKimarite(base){
  ...
  rows.forEach((row,i)=>{
    ...
    v = Math.max(1,v); v=Math.min(100,v);
...
function updateAnalysis(ai){
  ...
  let text="";
  if(main===1) text=`スタート踏み込む1コースが先マイ体勢。
握って回る${sub}コースが差しで続き内有利の決着濃厚。
${third}コースは展開待ちで三着争いまで。
外枠勢は展開向かず厳しい一戦となりそうだ。`;
  else if(main<=3) text=`${main}コースが攻めて主導権を奪う展開。
${sub}コースが内から抵抗し激しい攻防戦。
${third}コースが展開突いて浮上。
波乱含みのレース展開となりそうだ。`;
  else text=`外枠勢が果敢に仕掛け主導権争い。
...
function updateBets(ai){
  ...
  const candidates=[
    `${main}-${sub}-${third}`, `${main}-${third}-${sub}`, `${sub}-${main}-${third}`,
    `${sub}-${third}-${others[0]}`, `${sub}-${third}-${others[1]}`, `${sub}-${third}-${others[2]}`,
    `1-${main}-${sub}`, `1-${main}-${third}`, `1-${sub}-${third}`
  ];

  const uniqueSet = [...new Set(candidates)]; // 重複削除
  const cols=document.querySelectorAll(".bet-col");
  cols.forEach((col,j)=>{
    const items=col.querySelectorAll(".bet-item");
    items.forEach((el,i)=>{
      el.textContent = uniqueSet[j*3+i]||"";
    });
  });
}
...
function updateHitRateSimulation(base,predict,ai){
  ...
  rows.forEach((row,i)=>{
    ...
    row.querySelector(".hitrate-bar div").style.background = courseColors[i];
...
function updateTrustMeter(ai){
  ...
  solidity=Math.min(100,solidity); variance=Math.min(100,variance);
