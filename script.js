...
async function fetchRealOdds(){
  ...
  try{
    ...
    // サンプル（将来ここを公式APIやスクレイピングに差し替え）
    const url = "https://raw.githubusercontent.com/yourname/boat-ai-smart/main/odds_sample.json";

    const res = await fetch(url);
    const data = await res.json();

    // 仮：6艇分オッズ想定
    window.realOdds = data.odds;
