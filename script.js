document.body.insertAdjacentHTML(
  "afterbegin",
  "<div style='color:red;font-size:20px'>JS LOADED</div>"
);
// ===============================
// 24場名 + JSONファイル紐付け
// ===============================
...
function updateAnalysis(ai){
  ...
  // ★ デバッグ表示（最初に必ず出る）
  el.textContent = "🔍 updateAnalysis 起動確認";

console.log("realKimarite", window.realKimarite);

if(
  !Array.isArray(window.expectedValues) ||
  window.expectedValues.length !== 6 ||
  !Array.isArray(window.realKimarite) ||
  window.realKimarite.length !== 6
){
...
if (
  ...
  console.warn("realKimarite not ready", window.realKimarite);
}
...
try {
  const motor2Rate   = Number(window.motor2Rate)   || 0;
  const motorRecent3 = Number(window.motorRecent3) || 0;
  const isLowRank    = Boolean(window.isLowRank);

  if (
    motor2Rate >= 0.30 ||
    motorRecent3 >= 5 ||
    (isLowRank && motorRecent3 >= 4)
  ) {
    motorScore = 1;
  }
  else if (
  ...
  console.error("motor evaluation error", e);
}
