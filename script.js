...
setTimeout(()=>{
  ...
  boatBoxes.forEach((box,i)=>{
    ...
    if(window.expectedValues && window.expectedValues[i]){

      const ev = Number(window.expectedValues[i]);
      ...
      if(ev >= 0.5){
        ...
        // 濃さ計算（0.9〜1.5くらいを想定）
        let strength = Math.min(1, (ev - 0.9) / 0.6);

        // 緑を濃くする（透明度）
        let alpha = 0.3 + strength * 0.7;

        box.style.background = `rgba(46,204,113,${alpha})`;
        box.style.border = "2px solid #2ecc71";
        box.style.boxShadow = `0 0 ${8 + strength*12}px rgba(46,204,113,${alpha})`;
