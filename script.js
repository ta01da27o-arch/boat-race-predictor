...
function calcAll(){
  ...
  updateAnalysis(ai);
  updateBets(ai);
  updateHoleBets(ai);           // 穴買い目自動更新
  updateHitRateSimulation(base,predict,ai);
  updateTrustMeter(base,predict,ai); // 信頼度メーター更新
}
...
function updateExpectationBars(base,predict,ai){
  ...
  const colors = [
    "#ffffff","#000000","#ff3333","#3366ff","#ffcc00","#33cc66"
  ];
...
function updateHoleBets(ai){
  const sorted = ai
    .map((v,i)=>({v,i:i+1}))
    .sort((a,b)=>a.v-b.v); // 小さい順（穴候補）

  const main = sorted[0].i;
  const sub  = sorted[1].i;
  const third= sorted[2].i;

  const col = document.querySelectorAll(".bet-col")[3]; // 穴枠
  setCol(col,[
    `${main}-${sub}-${third}`,
    `${sub}-${third}-${main}`,
    `${third}-${main}-${sub}`
...
function updateHitRateSimulation(base,predict,ai){
  ...
  rows.forEach((row,i)=>{
    ...
    value.textContent = i+1 + "　" + hitRate + "%"; // コース番号＋%表示
...
// 信頼度メーター（C③完全版）
// ===============================
function updateTrustMeter(base,predict,ai){
  const meterBox = document.getElementById("trustMeter");
  if(!meterBox) return;

  const stability = Math.round(Math.max(...ai) - Math.min(...ai)); // 堅さスコア
  const chaos = Math.round(100 - stability);                        // 荒れ指数
  const trust = Math.round((Math.max(...ai) + stability)/2);        // 総合信頼度

  meterBox.innerHTML = `
    <div>堅さスコア : ${stability}</div>
    <div>荒れ指数 : ${chaos}</div>
    <div>総合信頼度 : ${trust}%</div>
  `;
    }
